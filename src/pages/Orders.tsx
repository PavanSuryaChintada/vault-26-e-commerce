  ...
  return (
    <div className="container-px py-24 min-h-screen bg-white">
      ...
      >
        ...
        {orders.length === 0 ? (
          <div className="text-center py-32 border border-black/5 bg-muted/20">
            <Package className="h-10 w-10 text-black/10 mx-auto mb-6" strokeWidth={1} />
            <p className="text-[10px] tracking-[0.3em] text-black/40 uppercase font-ui">Your archive contains no transactions.</p>
          ...
          <div className="space-y-6">
            {orders.map((o, i) => (
              ...
              >
                <Link to={`/orders/${o.id}`} className="group block border border-black/5 p-8 hover:border-black transition-all duration-500 bg-muted/20">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div className="flex-1">
                      ...
                      <div className="text-[10px] text-black/40 tracking-[0.1em] font-ui uppercase">
                    ...
                    <div className="flex items-center gap-8 w-full md:w-auto justify-between md:justify-end">
                      <div className="text-right">
                        ...
                        <span className="inline-block mt-2 text-[9px] uppercase tracking-[0.3em] text-black/40 font-ui font-bold">{o.status}</span>
                      </div>
                      <ArrowRight className="h-4 w-4 text-black/20 group-hover:text-black transition-all group-hover:translate-x-2" />
                    </div>
                  </div>
    ...
    <div className="container-px py-24 min-h-screen bg-white">
      ...
      >
        <header className="mb-16 border-b border-black/5 pb-12">
          ...
          <p className="text-[10px] tracking-[0.4em] text-black/40 font-ui mt-6 uppercase">PLOTTED ON {new Date(o.created_at).toLocaleDateString()} AT {new Date(o.created_at).toLocaleTimeString()}</p>
        </header>
            ...
            <div className="grid grid-cols-4 gap-4 mb-4">
              {STATUS_LABELS.map((s, i) => (
                <div key={s} className="flex flex-col gap-3">
                   <div className={cn('h-1 transition-all duration-1000', i <= stage ? 'bg-accent' : 'bg-black/5')} />
                   <span className={cn('text-[9px] tracking-[0.3em] font-ui font-bold uppercase', i <= stage ? 'text-black' : 'text-black/20')}>
                    {s}
                   </span>
                </div>
              ))}
            </div>
            <div className="flex items-center gap-4 text-[10px] tracking-[0.2em] font-ui text-black/40 uppercase">
        ...
        <div className="grid lg:grid-cols-[1fr_350px] gap-16">
          <section className="space-y-8">
            <div className="text-[10px] tracking-[0.5em] font-ui font-bold uppercase mb-8 border-b border-black/5 pb-4">Manifest</div>
            <div className="border border-black/5 bg-muted/20">
              {o.order_items.map((it: any) => (
                <div key={it.id} className="flex gap-6 p-6 border-b border-black/5 last:border-b-0">
          ...
          <aside className="space-y-12">
            <div>
              <div className="text-[10px] tracking-[0.5em] font-ui font-bold uppercase mb-8 border-b border-black/5 pb-4">Destination</div>
            ...
            <div>
              <div className="text-[10px] tracking-[0.5em] font-ui font-bold uppercase mb-8 border-b border-black/5 pb-4">Settlement</div>
