      ...
      .then(({ data }) => {
        setProducts(
          (data || []).map((p: any) => ({
            ...
            compare_price: p.product_variants?.[0]?.compare_price
              ? Number(p.product_variants[0].compare_price) : null,
            brand: p.brands?.name,
          }))
        );
      });
        ...
        <div className="max-w-[1600px] mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-14 md:mb-20">
            <div>
              ...
              >
                <span className="w-8 h-[1px] bg-accent" /> {cfg.eyebrow || 'DROP 02 — FRESH'}
              </motion.span>
              ...
              >
                {cfg.subtitle || 'Hand-picked drops, fresh in the archive.'}
              </motion.p>
            </div>
            ...
            >
              <Link
                to={cfg.cta_href || '/category/new-arrivals'}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-5 gap-y-12">
            {products.slice(0, cfg.product_count || 8).map((p, i) => {
                ...
                >
                  <Link to={`/products/${p.slug}`} className="block">
                    ...
                    <div className="pt-4 flex flex-col gap-1">
                      {p.brand && (
                        <div className="text-[10px] tracking-[0.3em] font-ui font-bold text-white/40 uppercase">{p.brand}</div>
                      )}
                      ...
                      <div className="mt-1 flex items-center gap-3">
                        ...
                        {onSale && (
                          <span className="text-white/30 line-through text-[10px] font-ui tracking-widest">{inr(p.compare_price!)}</span>
                        )}
                      </div>
                    </div>
                  </Link>
